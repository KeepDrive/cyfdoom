...
local function createConvertWarning()
...
local choice = 0
local function updateWarning(code)
    if choice == 0 then
...
local read = coroutine.create(function()
    if not pcall(Misc.OpenFile, "WADs/Converted/"..WADFileName, 'r') then
        createConvertWarning()
        coroutine.yield(1)
        WADString = convertWAD()
...
function Update()
    ...
    elseif code ~= 0 then
        code = updateWarning(code) or code
    end
end